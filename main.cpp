...
void Update_Sketch(BUCKET *bucket,int w)//Sketch的更新
{
    ...
    while(fread(&temp_key,1,KEY_SIZE,fp)==KEY_SIZE)
    {
        ...
        for(int i=0;i<d;i++)//控制哈希函数的循环
        {
            ...
            pos=i*w+index;
            minValue=min(minValue,bucket[pos].Count);
        }
        for(int i=0;i<d;i++)
        {
            Hash_Value=Hash[i]->run((const char*)&temp_key,KEY_SIZE);//根据BOBHash产生哈希值
            index=Hash_Value%w;//将哈希值投入二维表中，获取当前行的索引值
            pos=i*w+index;
            if(bucket[pos].Count==minValue)
            {
                bucket[pos].Count=bucket[pos].Count+1;
            }
        }
    }
}
...
int main()
{
    ...
    if(choose==0)
    {
        ...
        FILE *fp = fopen("CU Sktech Various_k.csv", "w+"); //创建csv文件用于存储数据
    ...
    else if(choose==1)
    {
        ...
        FILE *fp = fopen("CU Sktech Various_memory_k.csv", "w+"); //创建csv文件用于存储数据
    ...
    else if(choose==2)
    {
        ...
        FILE *fp = fopen("CU Sktech Various_T.csv", "w+"); //创建csv文件用于存储数据
    ...
    else if(choose==3)
    {
        ...
        FILE *fp = fopen("CU Sktech Various_memory_T.csv", "w+"); //创建csv文件用于存储数据
